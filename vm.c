...
void vm_init (struct VM *vm) {
    ...
    vm_load_instruction(vm, 0x05, "call",   ' ', 'i', vm_instruction_call);
    vm_load_instruction(vm, 0x06, "call",   ' ', 'r', vm_instruction_call);
...
void vm_debug_op (struct VM *vm) {
    printf("0x%04x %s ", vm->pc, vm_debug_opcode2name(vm, vm->opcode));
...
void vm_step (struct VM *vm) {
    ...
    if (vm->opcodes[vm->opcode].func(vm) == 0) {
        ...
        //we jumped in the operation, leave PC alone
    }
    #ifdef DEBUG
